...
```bash
curl -O https://cnb.cool/imagehubcc/reinstall/-/git/raw/main/reinstall.sh || wget -O ${_##*/} $_
```
...
```batch
certutil -urlcache -f -split https://cnb.cool/imagehubcc/reinstall/-/git/raw/main/reinstall.bat
```
...
- **Ubuntu 支持自定义分区**：自动创建 `/boot` (2GB)、`/` (默认 20GB，可通过 `--root-size` 自定义)、`/data` (剩余空间) 三个分区
