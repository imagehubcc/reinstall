...
[![Codacy](https://img.shields.io/codacy/grade/dc679a17751448628fe6d8ac35e26eed?logo=Codacy&label=Codacy&style=flat-square)](https://app.codacy.com/gh/imagehubcc/reinstall/dashboard)
[![CodeFactor](https://img.shields.io/codefactor/grade/github/imagehubcc/reinstall?logo=CodeFactor&logoColor=white&label=CodeFactor&style=flat-square)](https://www.codefactor.io/repository/github/imagehubcc/reinstall)
[![Lines of Code](https://tokei.rs/b1/github/imagehubcc/reinstall?category=code&label=Lines%20of%20Code&style=flat-square)](https://github.com/XAMPPRocky/tokei_rs)
...
[![GitHub Issues](https://img.shields.io/badge/GitHub-%23121011.svg?style=for-the-badge&logo=github&logoColor=white)](https://github.com/imagehubcc/reinstall/issues)
...
| <img width="16" height="16" src="https://github.com/imagehubcc/reinstall/assets/7548515/f74b3d5b-085f-4df3-bcc9-8a9bd80bb16d" /> Kali                                                                                                                                                                                                                                   | 滚动                                  | 256 MB    | 1 ~ 1.5 GB ^ |
...
```bash
curl -O https://raw.githubusercontent.com/imagehubcc/reinstall/main/reinstall.sh || wget -O ${_##*/} $_
```
...
- <https://raw.githubusercontent.com/imagehubcc/reinstall/main/reinstall.bat>
...
```batch
certutil -urlcache -f -split https://raw.githubusercontent.com/imagehubcc/reinstall/main/reinstall.bat
```
...
- **Ubuntu 支持自定义分区**：自动创建 `/boot` (2GB)、`/` (默认 20GB，可通过 `--root-size` 参数自定义)、`/data` (剩余空间) 三个分区，详见 [Ubuntu 自定义分区](#ubuntu-自定义分区)
...
- `--root-size SIZE` 设置 Ubuntu 根分区大小（仅限 Ubuntu），支持格式：`20G`、`20GB`、`20480M`、`20480MB`、`20GiB`、`20480MiB`。如果只输入数字（如 `40`），默认按 GB 解析。不指定时默认 20GB，如果磁盘空间不足则自动使用磁盘的 30%
...
- **`/boot` 分区**：固定 2GB，用于存放内核和引导文件
- **`/` 根分区**：默认 20GB，可通过 `--root-size` 参数自定义大小
...
```bash
# 使用默认 20GB 根分区
bash reinstall.sh ubuntu 24.04 --password your_password

# 自定义根分区大小为 40GB
bash reinstall.sh ubuntu 24.04 --password your_password --root-size 40G

# 自定义根分区大小为 40GB（不带单位，默认按 GB 解析）
bash reinstall.sh ubuntu 24.04 --password your_password --root-size 40
```

> [!NOTE]
>
> - 此功能仅适用于 Ubuntu 系统安装（支持所有 Ubuntu 版本）
> - 根分区大小支持多种格式：`20G`、`20GB`、`20480M`、`20480MB`、`20GiB`、`20480MiB`
...
![Windows 安装界面](https://github.com/imagehubcc/reinstall/assets/7548515/07c1aea2-1ce3-4967-904f-aaf9d6eec3f7)
...
![image-name](https://github.com/imagehubcc/reinstall/assets/7548515/5aae0a9b-61e2-4f66-bb98-d470a6beaac2)
...
从 <https://github.com/imagehubcc/reinstall/commits/main> 右侧找到旧版本的 `commit_id`

```bash
commit_id=xxxxxxx
curl -O https://raw.githubusercontent.com/imagehubcc/reinstall/$commit_id/reinstall.sh || wget -O ${_##*/} $_
...
1. Fork 本仓库或创建新仓库
2. 修改 `reinstall.sh` 和 `reinstall.bat` 开头的 `confhome`，指向你的仓库地址
